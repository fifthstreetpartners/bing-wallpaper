...
{
    public partial class BWApplicationContext : ApplicationContext
    {
        private BingImageProvider _provider;
        private Settings _settings;
        private Image _currentWallpaper;
        private int _currentIndex = 0;
        private MenuItem _nextImageMI;
        ...
        /// </summary>
        public async void SetWallpaper(int idx = 0)
        {
            try
            {
                var bingImg = await _provider.GetImage(idx);
        ...
        public void AddTrayIcons()
        {
            ...
            // Force update button
            _trayMenu.MenuItems.Add("Force Update", (s, e) => { CurrentIndex = 0; SetWallpaper(CurrentIndex); });

            // Previous Image
            _trayMenu.MenuItems.Add("Previous Image", (s, e) => SetWallpaper(CurrentIndex+=1));

            // Next Image
            _nextImageMI = _trayMenu.MenuItems.Add("Next Image", (s, e) => SetWallpaper(CurrentIndex -= 1));
        ...
        private void _trayMenu_Popup(object sender, EventArgs e)
        {
            if (CurrentIndex > 0)
            {
                _nextImageMI.Enabled = true;
            }
            else
            {
                _nextImageMI.Enabled = false;
            }
        }
