...
namespace BingWallpaper
{
    ...
    {
        public async Task<BingImage> GetImage(int idx = 0)
        {
            string baseUri = "https://www.bing.com";
            using (var client = new HttpClient())
            {
                using (var jsonStream = await client.GetStreamAsync($"http://www.bing.com/HPImageArchive.aspx?format=js&idx={idx}&n=1&mkt=en-US"))
                {
